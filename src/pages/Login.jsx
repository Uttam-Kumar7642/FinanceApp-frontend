...
// Pure reducer — error ONLY changes on explicit SET_ERROR or CLEAR_ERROR action
...
export default function Login() {
  ...
  const handleSubmit = async (ev) => {
    ...
    // Validate — sets error and STOPS, does NOT clear existing error first
    ...
    try {
      // Call API directly — do NOT use useAuth login() to avoid any side-effect clearing
      const res = await axios.post('/api/auth/login', { email, password });
      const { token, ...userData } = res.data;
      localStorage.setItem('token', token);
      localStorage.setItem('user', JSON.stringify(userData));
      // Navigate — error never gets cleared, but page changes anyway
      ...
      if (status === 401 || msg.toLowerCase().includes('invalid') || msg.toLowerCase().includes('password') || msg.toLowerCase().includes('incorrect')) {
        ...
        dispatch({ type:'SET_ERROR', value:'Cannot connect to server. Check your internet connection.' });
      } else {
        dispatch({ type:'SET_ERROR', value: msg || 'Something went wrong. Please try again.' });
      }
    }
    // NOTE: loading is set to false inside SET_ERROR — never in finally{}
    // This prevents a re-render that could wipe the error
  };
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      ...
      <div style={{ width:'100%', maxWidth:420, animation:'fadeIn 0.4s ease' }}>
        ...
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:20, padding:'32px 28px', boxShadow:'var(--shadow-lg)' }}>
          ...
          {/* ERROR BANNER — rendered from state.error, only removed by CLEAR_ERROR action or manual X */}
          {error && (
            <div style={{
              display:'flex', alignItems:'flex-start', gap:10,
              background:'rgba(255,85,105,0.13)',
              border:'1.5px solid rgba(255,85,105,0.5)',
              borderRadius:10, padding:'13px 14px', marginBottom:20,
            }}>
              <AlertCircle size={17} color="#ff5569" style={{ flexShrink:0, marginTop:1 }}/>
              <span style={{ fontSize:14, color:'#ff5569', lineHeight:1.6, flex:1, fontWeight:500 }}>
                {error}
              </span>
              <button
                onClick={() => dispatch({ type:'CLEAR_ERROR' })}
                style={{ background:'none', border:'none', cursor:'pointer', color:'#ff5569', padding:0, flexShrink:0, display:'flex' }}
              >
                <XCircle size={16}/>
              </button>
            </div>
          ...
          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:16 }} noValidate>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              ...
              <div style={{ position:'relative' }}>
                ...
                <button
                  type="button"
                  onClick={() => dispatch({ type:'TOGGLE_PASS' })}
